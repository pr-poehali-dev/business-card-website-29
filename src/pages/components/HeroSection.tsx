  ...
  return (
    <>
      ...
      <section id="hero" className="relative overflow-hidden pt-[70px]"
        ...
        {/* Десктоп: тёмный слой снизу */}
        <div className="absolute inset-0 hidden sm:block" style={{
          zIndex: 1,
          background: "linear-gradient(to top, rgba(10,14,24,1) 0%, rgba(10,14,24,0.4) 25%, transparent 55%)",
        }} />

        {/* Десктоп: контент по центру */}
        <div className="relative hidden sm:flex max-w-screen-xl mx-auto px-5 flex-col justify-center"
          ...
          <div className="max-w-2xl">
            ...
            <p className="text-lg leading-relaxed mb-5 max-w-xl" style={{ color: C.light }}>
        ...
        {/* Мобиль: текст в нижней части, фото занимает верх */}
        <div className="sm:hidden absolute bottom-0 left-0 right-0 px-4 pb-8" style={{ zIndex: 5 }}>
          {/* Градиент только под текстом */}
          <div className="absolute inset-0" style={{
            background: "linear-gradient(to top, rgba(10,14,24,1) 0%, rgba(10,14,24,0.97) 60%, rgba(10,14,24,0.6) 85%, transparent 100%)",
            zIndex: -1,
          }} />
          <h1 className="font-black leading-[1.05] mb-3" style={{ fontSize: "clamp(1.7rem,8vw,2.4rem)", textShadow: "0 2px 20px rgba(0,0,0,0.9)" }}>
            Профессиональное<br />
            <span style={{ color: C.cyan }}>асфальтирование</span><br />
            в Нижнем Новгороде
          </h1>
          <div className="inline-flex items-center gap-2.5 rounded-xl px-4 py-2.5 mb-4"
            style={{ background: "rgba(240,192,48,0.12)", border: `1px solid rgba(240,192,48,0.4)` }}>
            <Icon name="Layers" size={16} style={{ color: C.gold } as React.CSSProperties} />
            <div>
              <div className="text-[8px] font-bold uppercase tracking-widest" style={{ color: C.muted }}>Стоимость работ</div>
              <div className="font-black text-lg" style={{ color: C.gold }}>от 1 200 ₽/м²</div>
            </div>
          </div>
          <div className="flex flex-col gap-3">
            <a href={PHONE_HREF}
              className="flex items-center justify-center gap-2 font-black text-sm px-6 py-4 rounded-full active:scale-95"
              style={{ background: `linear-gradient(135deg,${C.goldDark},${C.gold})`, color: "#000", boxShadow: "0 4px 20px rgba(240,192,48,0.4)" }}>
              <Icon name="Phone" size={16} />
              Позвонить: {PHONE}
            </a>
            <button onClick={() => go("form")}
              className="font-black text-sm px-6 py-4 rounded-full border"
              style={{ borderColor: "rgba(255,255,255,0.25)", color: "#fff", background: "rgba(255,255,255,0.07)" }}>
              Оставить заявку
            </button>
          </div>
        </div>
